...
export async function initiateCallback(request, student, teacher) {
  try {
    ...
    // Prepare a simplified payload for Bland AI
    const payload = {
 ...
 * @returns {object} - Call status information
 */
export async function getCallStatus(callId) {
  try {
    const response = await blandAIClient.get(`/v1/calls/${callId}`);
    return response.data;
  } catch (error) {
    console.error('Error getting call status from Bland AI:', error);
    throw error;
  }
}

/**
 * End an ongoing Bland AI call
 * @param {string} callId - The Bland AI call ID
 * @returns {object} - Response from Bland AI
 */
export async function endCall(callId) {
  try {
    const response = await blandAIClient.post(`/v1/calls/${callId}/end`);
    return response.data;
  } catch (error) {
    console.error('Error ending call with Bland AI:', error);
    throw error;
  }
}