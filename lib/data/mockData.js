...
export const createCallbackRequest = (request) => {
  // Generate a unique ID using our counter and increment it
  const id = `cr${nextRequestId++}`;
  ...
  // Add to the existing requests array
...
export const fixDuplicateRequestIds = () => {
  console.log("Checking for duplicate request IDs...");
  const idMap = new Map();
  let fixCount = 0;
  
  callbackRequests.forEach(request => {
    if (idMap.has(request.id)) {
      // Found a duplicate - assign a new unique ID
      const oldId = request.id;
      request.id = `cr${nextRequestId++}`;
      console.log(`Fixed duplicate ID: ${oldId} → ${request.id}`);
      fixCount++;
    } else {
      // Mark this ID as seen
      idMap.set(request.id, true);
      ...
      if (request.id.startsWith('cr')) {
        const idNumber = parseInt(request.id.substring(2));
        if (!isNaN(idNumber) && idNumber >= nextRequestId) {
          nextRequestId = idNumber + 1;
        }
      }
  ...
  console.log(`Fixed ${fixCount} duplicate IDs. Next ID will be cr${nextRequestId}`);
  return fixCount;
};
fixDuplicateRequestIds();
